...
namespace WashCar.API.Application.Models
{
    public class AppointmentsDetailsDTO
    {
        ...
        public int SchedulesCancelled => Schedules?.Count(s => s.Status == ScheduleStatusEnum.Canceled.ToString()) ?? 0;
        public int SchedulesPending => Schedules?.Count(s => s.Status == ScheduleStatusEnum.Pending.ToString()) ?? 0;
        public List<ScheduleDetailsDTO> Schedules { get; set; } = new List<ScheduleDetailsDTO>();
    }
